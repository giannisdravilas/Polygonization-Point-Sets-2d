...
polygon_is_ok simulated_annealing_algorithm(string inputFile, string outputFile, bool maximization_problem, Polygon_2 startPolygon, int L, string annealing, Points pointsToIgnore, double max_time = 0){
    ...
    while((T >=0) || (maxIterations == 0)){
        ...
        if ((chrono::duration_cast<chrono::milliseconds>(done-started).count() >= max_time )&& (max_time != 0)){
            // cout <<"simulated_annealing_algorithm-timeout" << endl;
            if (startPolygon.is_clockwise_oriented()){
                startPolygon.reverse_orientation();
            }
            return polygon_is_ok{startPolygon, false};
        }
    ...
    return polygon_is_ok{startPolygon, true};
}
...
polygon_is_ok simulated_annealing_algorithm_subdivision(string inputFile, string outputFile, bool maximization_problem, int L, string annealing, int edgeSelection, string startAlgorithm, double max_time = 0){
    ...
    std::chrono::_V2::system_clock::time_point started_for_output, done_for_output;
    ...
    started_for_output = chrono::high_resolution_clock::now();// Start time
    ...
    for(int i=0;i<vectorOfPolygons.size();i++){
        ...
        // Call Global Step
        polygon_is_ok tempPoly2 = simulated_annealing_algorithm(inputFile, to_string(i) +outputFile, maximization_problem, temptemppoly, L, "global", pointsToIgnore, 0);
        Polygon_2 tempPoly = tempPoly2.polygon;
    ...
    if ((chrono::duration_cast<chrono::milliseconds>(done-started).count() >= max_time )&& (max_time != 0)){
        return polygon_is_ok{LastPolygon, false};
    }

    // Call Local Step
    polygon_is_ok endPolygon2 = simulated_annealing_algorithm(inputFile, outputFile +".txt", maximization_problem, LastPolygon, L, "local", Points(), max_time - chrono::duration_cast<chrono::milliseconds>(done - started).count());
    Polygon_2 endPolygon = endPolygon2.polygon;
    // reverse back
    ...
    if ((chrono::duration_cast<chrono::milliseconds>(done-started).count() >= max_time )&& (max_time != 0)){
        return polygon_is_ok{endPolygon, false};
    }

    // Write in output file
    done_for_output = chrono::high_resolution_clock::now(); // end timer
    outputForSA(inputFile, outputFile, endPolygon, maximization_problem, initial_polygon_area, started_for_output, done_for_output, annealing);
    return polygon_is_ok{endPolygon, true};
}
...
void make_small_polygons_and_mark_segments(string inputFile, vector<Polygon_2>* vectorOfPolygons, vector<pair<Segment_2, Segment_2>>* vectorOftLowerHullEdge){
    ...
    int m;// Number of points in each subset
    if(tempPoints.size() > 1000 ){
        m = tempPoints.size()/10;
    }else if ((tempPoints.size()>50) && (tempPoints.size()<=1000)){
        m = tempPoints.size()/5;
    }else if ((tempPoints.size()>10) && (tempPoints.size()<=50)){
        m = tempPoints.size()/2;
    }else{
        m = tempPoints.size();
        vectorOfPolygons->push_back(Polygon_2(tempPoints.begin(), tempPoints.end()-1));
        return;
    }
...
Points make_points_to_ignore(int i, int size, vector<pair<Segment_2, Segment_2>> vectorOftLowerHullEdge){
    ...
    if((i==0) && (i == size-1)){
        return Points();
    }else if(i==0){// If we are at the first polygon, check only right
