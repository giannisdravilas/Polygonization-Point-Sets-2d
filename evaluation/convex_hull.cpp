...
polygon_is_ok convex_hull_algorithm(string inputFile, string outputFile, int edgeSelection, double max_time = 0){
    ...
    do{
        ...
        if ((chrono::duration_cast<chrono::milliseconds>(done_for_assignment_3-started_for_assignment_3).count() >= max_time )&& (max_time != 0)){
            return polygon_is_ok{MyPolygon, false};
        }
        ...
        if(isEdgeVisible == false){
            ...
            if(mapOfNearestPointsLeft.size() == 0){
                //cout << "[Error: Could not find visible edge, exiting...] ["<<diffVector.size()<<"]" << endl;
                return polygon_is_ok{MyPolygon, false};
            }
            continue;
        }
    ...
    cout << fixed << "Polygon area is Convex Hull: " << MyPolygon.area() << endl;
    ...
    cout<< "Construction Time Convex Hull: " << chrono::duration_cast<chrono::milliseconds>(done-started).count() << " ms" << endl;
    outputFileClass.close();
    
    return polygon_is_ok{MyPolygon, true};
}